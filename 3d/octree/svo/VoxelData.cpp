...
VoxelData::VoxelData(const char *path, size_t mem)
//        : _loader(0)
{
...
/**
 * Counts the number of nodes in an octree of numLevels including
 * intermediate levels
 * @param numLevels
 * @return
 */
...
/**
 * Builds a collection of offsets to the levels of the grid
 * @param base
 * @param numLevels
 * @param dst
 */
...
void VoxelData::upsampleLutLevel(int l) {
    ...
    if (size < 128) {
        ...
        // Disabled because of compilation problem with threads
...
/**
 * Initializes the VoxelData
 * @param mem available system memory to work
 */
void VoxelData::init(size_t mem) { // mem: 53137200 bytes (50 mbytes)
    // Calculate virtual memory sizes, round data to the next power of 2
    _virtualDataW = roundToPow2(_dataW); // _dataW 1000 // _virtualDataW 1024
    _virtualDataH = roundToPow2(_dataH); // _dataH 1000 // _virtualDataH 1024
    _virtualDataD = roundToPow2(_dataD); // _dataD 1000 // _virtualDataD 1024
    //_highestVirtualBit = 10
...
//        _cellCost += _loader->blockMemRequirement(1, 1, 1);
    ...
    // Caculates cost for a specific partition level
    auto partitionCost = [&](int level) { // level 7
        int lowBit = _highestVirtualBit - level; // lowBit = 3
        uint64 topLutCost = countCellsInHiarchicalGrid(level + 1); // topLutCost = nodos en los niveles 10 a 8
        uint64 lowLutCost = countCellsInHiarchicalGrid(lowBit); // lowLutCost = nodos en los niveles 7 a 0
        uint64 gridCost = (uint64(1) << uint64(lowBit*3))  *_cellCost; // coste de los voxeles cargados? 2^lowbit^3
        return topLutCost + lowLutCost + gridCost;
    };

    // calculate largest lower level that fits in memory, smaller levels
    // fill more memory
    ...
    _lowLutLevels = largestLowerLevel; // levels in lower lut
    _topLutLevels = _highestVirtualBit - largestLowerLevel + 1; // levels in top lut
    ...
    // buffered data (grid) 2^largestLowerLevel^3
